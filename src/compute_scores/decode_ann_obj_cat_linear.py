...
"""
Decode Object Category from ANN Model Features using Linear Classification
This script processes ANN (Artificial Neural Network) model features to classify object
category using Linear Discriminant Analysis (LDA) with 10-fold cross-validation.
The script:
1. Loads pre-computed ANN model features from an HDF5 file with shape [n_frames, videos, units, model_repetitions]
2. Loads object category labels corresponding to each video
...
import h5py
import numpy as np
from utils.classification import get_classifications, compute_i1
...
def compute_scores(process_id, parallelizations, classification_scores, model_features, object_labels, split_repetitions, start, end):
    ...
    for split_rep, time_bin, model_rep in parallelizations[start:end]:
        ...
        predictions, probabilities = get_classifications(current_data, object_labels, nclasses=8, nrfolds=10, model='lda', seed=np.random.randint(0, 1000), standardize=True)
        i1_scores, i1_std, per_class_scores = compute_i1(probabilities, object_labels, return_scores=True)
        
        # Store per-class scores
        classification_scores[split_rep, time_bin, :, model_rep] = per_class_scores
...
if __name__ == "__main__":
    ...
    # Load object category labels indexed by video
    ...
    for i in range(num_processes):
        ...
        p = mp.Process(target=compute_scores, args=(i, parallelizations, classification_scores, 
    ...
    save_file_name = f"decode_ann_obj_cat_linear.h5" 
