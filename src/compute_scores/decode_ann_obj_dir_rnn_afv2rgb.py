...
training on AFV features and testing on both AFV and RGB features.
The script:
...
4. For each combination of (split_repetition, time_bin, model_repetition):
    ...
    - Performs RNN classification with 10-fold cross-validation training on AFV features and testing on both AFV and RGB features
...
Input:
     - RGB model features HDF5 file: [n_frames, videos, units, model_repetitions]
     - AFV model features HDF5 file: [n_frames, videos, units, model_repetitions]
     - Object direction labels: 1D array of class labels indexed by video
Output:
     - HDF5 file containing I1 classification scores for both RGB and AFV with shape [split_repetitions, n_frames, videos, model_repetitions]
"""
import sys
sys.path.append('../')
...
def compute_scores(process_id, parallelizations, classification_scores_rgb, classification_scores_afv, model_features_rgb, model_features_afv, object_labels, model_config, split_repetitions, start, end):
    ...
    for split_rep, time_bin, model_rep in parallelizations[start:end]:
        ...
        current_data_rgb = model_features_rgb[:, :, :, model_rep]  # shape: [time_bins, videos, sites]
        current_data_afv = model_features_afv[:, :, :, model_rep]  # shape: [time_bins, videos, sites]
        current_data_rgb = current_data_rgb[:time_bin+1]  # shape: [videos, sites]
        current_data_afv = current_data_afv[:time_bin+1]  # shape: [videos, sites]
        ...
        classification_scores_rgb[split_rep, time_bin, :, model_rep] = i1_rgb_all
        classification_scores_afv[split_rep, time_bin, :, model_rep] = i1_afv_all
...
if __name__ == "__main__":
    ...
    model_features_rgb_path = '[your file containing ANN model features for RGB videos]'
    model_features_afv_path = '[your file containing ANN model features for AFV videos]'
    split_repetitions = 10
    hpc = True
    scores_save_path = '../../scores'

    features_rgb_filename = model_features_rgb_path.split('/')[-1]
    features_rgb_file = h5py.File(model_features_rgb_path, 'r')
    print("Keys:", list(features_rgb_file.keys()))
    # INPUT FILE SHAPE: [n_frames, videos, units, model repetitions]
    model_features_rgb = np.array(features_rgb_file['features'])

    features_afv_filename = model_features_afv_path.split('/')[-1]
    features_afv_file = h5py.File(model_features_afv_path, 'r')
    print("Keys:", list(features_afv_file.keys()))
    # INPUT FILE SHAPE: [n_frames, videos, units, model repetitions]
    model_features_afv = np.array(features_afv_file['features'])
   
    n_time_bins = model_features_rgb.shape[0]
    ...
    for split_rep in range(split_repetitions):
        for time_bin in range(n_time_bins):
            for model_rep in range(model_features_rgb.shape[3]):
    ...
    assert object_labels.shape[0] == model_features_rgb.shape[1]
    ...
    classification_scores_rgb = m.np_empty((split_repetitions, model_features_rgb.shape[0], model_features_rgb.shape[1], model_features_rgb.shape[3]))
    classification_scores_afv = m.np_empty((split_repetitions, model_features_afv.shape[0], model_features_afv.shape[1], model_features_afv.shape[3]))
    ...
    for i in range(num_processes):
        ...
        p = mp.Process(target=compute_scores, args=(i, parallelizations, classification_scores_rgb, classification_scores_afv,
                                                     model_features_rgb, model_features_afv, object_labels, model_config,
    ...
    save_file_name = f"decode_ann_obj_dir_rnn_afv2rgb.h5" 
