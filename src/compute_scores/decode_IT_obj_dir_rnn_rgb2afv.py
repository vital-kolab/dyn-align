...
4. For each combination of (split_repetition, time_bin):
    ...
    - Performs RNN classification with 10-fold cross-validation training on RGB neural data and testing on both RGB and AFV neural data
...
def compute_scores(process_id, parallelizations, classification_scores_rgb, classification_scores_afv, neural_recordings_rgb, neural_recordings_afv, object_labels, split_repetitions, start, end):
    ...
    for split_rep, time_bin in parallelizations[start:end]:
        ...
        preds_rgb, probs_rgb, preds_afv, probs_afv = get_rnn_classifications_rgb_afv(current_data_rgb, current_data_afv, object_labels, nfolds=10, seed=np.random.randint(1000), standardize=True)
